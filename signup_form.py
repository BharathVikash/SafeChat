...
class SignUpForm(ft.UserControl):
    ...
    def btn_signup(self, e):
        ...
        if not self.text_password.value:
            ...
            self.submit_values(self.text_user.value,
                               self.text_password.value, self.text_mobile.value)
    ...
    def build(self):
        ...
        self.text_mobile = ft.TextField(label="Mobile")
        self.text_signup = ft.ElevatedButton(
            text="Sign up", color=ft.colors.WHITE, width=150, height=50, on_click=self.btn_signup)
        self.text_signin = ft.Row(controls=[ft.Text(value="Already have an account?"), ft.TextButton(
            text="Sign in", on_click=self.btn_signin)], alignment=ft.MainAxisAlignment.CENTER)
